...
Lives in src/ (not scripts/) because the deployed API imports it: the judge-
facing "Restore demo" button calls restore_hero() at runtime, and the Docker
image copies only src/ and contract/. scripts/demo_restore_hero.py is a thin
CLI wrapper over this module.

Idempotent: clears any existing reconstruction/decision chain for the hero
first, then reseeds.
"""
